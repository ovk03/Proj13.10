    ...
    def render3d(self, buffer=None, cache: bool = False) -> bool:
        ...
        if RENDER_CORE_COUNT > 1:
            ...
            for i in range(RENDER_CORE_COUNT):
                ...
                cores.append(p.apply_async(self.multi_core_rendrer,(self.camera_pos, self.camera_rot, screen_width, screen_height, canvas,buffer[start:stop],)))
            tcl_code=[]
            for c in cores:
                tcl_code.extend(c.get())
            return GameToTK().draw_code("".join(tcl_code))
        else:
            return self.single_core_rendrer(buffer)
